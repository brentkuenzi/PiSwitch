...
class MyForm(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.panel_two.SetBackgroundColour('#3C3C3B') # #3C3C3B

        right = wx.Bitmap("NewTouchBoot/right22.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.rcon = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=right, pos=(width/2 + 220, 100))
        left = wx.Bitmap("NewTouchBoot/left22.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.lcon = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=left, pos=(width/2 - 350, 100))
        middle = wx.Bitmap("NewTouchBoot/Retroswitch_icon.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.switch = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=middle, pos=(width/2-120, 100))
        middle_l = wx.Bitmap("NewTouchBoot/Retroswitch_icon_left.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.lcheck = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=middle_l, pos=(width/2-120, 100))
        middle_r = wx.Bitmap("NewTouchBoot/Retroswitch_icon_right.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.rcheck = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=middle_r, pos=(width/2-120, 100))
        middle_b = wx.Bitmap("NewTouchBoot/Retroswitch_icon_both.png", wx.BITMAP_TYPE_ANY)
        self.panel_two.bcheck = wx.StaticBitmap(self.panel_two, id=wx.ID_ANY, bitmap=middle_b, pos=(width/2-120, 100))
        ...
        txt1 = "To connect Joy-Con controllers"
        txt2 = "press the plus and minus buttons."
        txt3 = "The RetroSwitch icon will color when successful"
        txt = txt1+"\n"+txt2+"\n"+txt3
        font = wx.Font(18, wx.ROMAN, wx.MODERN, wx.BOLD)
        self.panel_two.lbl.SetFont(font)
        self.panel_two.lbl.SetLabel(txt)
        self.panel_two.lbl.SetForegroundColour(wx.WHITE)

        check = wx.Bitmap("NewTouchBoot/check.png", wx.BITMAP_TYPE_ANY)
        ...
        #self.panel_two.lcon.Hide()
        #self.panel_two.rcon.Hide()
        self.panel_two.rcheck.Hide()
        self.panel_two.lcheck.Hide()
        self.panel_two.bcheck.Hide()
        ...
        rpbmp = wx.Bitmap("NewTouchBoot/retro.jpg", wx.BITMAP_TYPE_ANY)
        rpbmp = scale_bitmap(rpbmp, width/2, height/2)
        kbbmp = wx.Bitmap("NewTouchBoot/kodi.jpg", wx.BITMAP_TYPE_ANY)
        kbbmp = scale_bitmap(kbbmp, width/2, height/2)
        rbbmp = wx.Bitmap("NewTouchBoot/Debian.jpg", wx.BITMAP_TYPE_ANY)
        rbbmp = scale_bitmap(rbbmp, width/2, height/2)
        tbbmp = wx.Bitmap("NewTouchBoot/terminal.png", wx.BITMAP_TYPE_ANY)
    ...
    def OnTimer(self, event):
        ...
        global jconl
        if jconr is True:
            self.panel_two.switch.Hide()
            self.panel_two.rcheck.Show()
        if jconl is True:
            self.panel_two.switch.Hide()
            self.panel_two.lcheck.Show()
            if jconr is True:
                self.panel_two.switch.Hide()
                self.panel_two.rcheck.Hide()
                self.panel_two.lcheck.Hide()
                self.panel_two.bcheck.Show()
    ...
    def PressedOnePlayer(self, e):
        fileopen('NewTouchBoot/mounts')
        sleep(0.1)
        f = open("NewTouchBoot/checknum","w")
        f.write("num=1")
        f.close()
        exit()
